...
const Profile = () => {
    ...
    useEffect(() => {
        if (!notifSocket || !targetId) return;

        const handleNewFollow = (data) => {
            if (data.follower_id === targetId || data.following_id === targetId) {
                fetchProfileData();
            }
        };

        const handleDeletePost = (data) => {
            setPosts(prev => prev.filter(p => (p.id !== data.postId && p._id !== data.postId)));
        };
